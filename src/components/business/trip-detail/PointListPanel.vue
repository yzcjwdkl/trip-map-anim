<template>
  ...
  <Teleport to="body">
    ...
    <!-- 侧边面板 -->
    <div class="side-panel point-list-panel" ref="panelRef">
      <div class="side-panel-header">
        <span class="side-panel-title">点位列表</span>
      ...
      <div class="side-panel-body" ref="bodyRef">
        <slot />
      </div>
    </div>
  </Teleport>
...
watch(() => props.visible, (val) => {
  ...
  if (val) {
    ...
    // 内容错开淡入
    if (bodyRef.value) {
      gsap.fromTo(bodyRef.value.querySelectorAll('.panel-item'),
        { opacity: 0, x: 20 },
        { opacity: 1, x: 0, stagger: 0.03, duration: 0.4, ease: 'power3.out', delay: 0.2 }
      )
    }
  } else {
    // 退出动画
