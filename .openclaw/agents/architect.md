...
- GSAP 3.12+（ScrollTrigger + SplitText）→ 可调用 `gsap-frameworks`、`gsap-performance`、`gsap-scrolltrigger` skills 获取详细知识
- 高德地图通过 `@amap/amap-jsapi-loader` → 可调用 `amap-jsapi-skill` 获取详细知识
...
├── styles/
│   ├── tailwind.css
│   └── design-token.css   # CSS 变量映射 design-token.md
├── utils/
└── App.vue
public/
├── images/
...
### GSAP 强制规范
- 必须在 `main.js` 注册 `gsap.registerPlugin(ScrollTrigger)`
- **禁止** 在组件外创建全局 Tween
- 组件 `onUnmounted` 必须 `ScrollTrigger.getAll().forEach(t => t.kill())` 或 `ctx.revert()`
- 移动端（`< 768px`）默认禁用复杂 ScrollTrigger，改用简单 fade
- 默认 `ease: "power2.out"`, `duration: 0.6`

### 高德地图强制规范
- API Key 必须走 `.env`：`import.meta.env.VITE_AMAP_KEY`
- 必须封装为 `MapContainer.vue`，Props：`center`, `zoom`, `markers`
- 组件卸载必须 `map.destroy()`
- 加载失败降级：静态图片 + "点击加载地图"按钮
...
处理技术方案时，主动调用：
- `vue-best-practices` — Vue 3 架构规范、Composition API 最佳实践
- `gsap-frameworks` — GSAP 与 Vue 集成模式、生命周期管理
- `gsap-performance` — 动画性能优化、避免 layout thrashing
- `amap-jsapi-skill` — 高德地图 JS API 封装、DOM 获取规范
- `karpathy-guidelines` — 代码质量规范、避免过度设计

调用方式：在制定架构约束时，通过自然语言描述触发，如"参考 gsap-performance skill，制定移动端降级策略"。