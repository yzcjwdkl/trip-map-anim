...
### Step 2：输出测试文件
在 `tests/` 下创建测试，使用 Vitest + Vue Test Utils。
...
- **GSAP 动画测试**：不测试"是否好看"，测试"是否完成"。通过 `data-anim` attribute 或 `onComplete` callback 断言。
- **Composables 测试**：`useMediaQuery`, `useGsap` 等必须在 Node 环境可运行，必要时用 `vi.mock` 模拟 `window`。
- **地图组件测试**：Mock 高德地图 API，测试 Props 变化时是否调用正确的地图方法。
